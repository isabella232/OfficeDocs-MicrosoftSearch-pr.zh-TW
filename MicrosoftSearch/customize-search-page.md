...
ms.locfileid: "58639586"
---
# <a name="customize-the-search-results-page"></a>自訂搜尋結果頁面

您可以建立 *搜尋* 類別和 *結果類型*，以自訂使用者在 Microsoft [SharePoint](https://sharepoint.com/)、 [Microsoft Office](https://office.com)及 Microsoft 搜尋中的 [Bing](https://bing.com)搜尋時所看到的搜尋結果。 縱向使用者可讓使用者更輕鬆地找到他們具有查看許可權的資訊。

例如，您可以為行銷部門中的使用者建立協力廠商軟體的「行銷分析」資料的搜尋類別。 您也可以定義結果類型及自訂此資料的版面配置。

## <a name="about-search-verticals"></a>關於搜尋縱向

Microsoft 搜尋結果] 頁面的頂端有一列索引標籤。 這些是搜尋行業。 搜尋類別只會顯示特定類型或特定內容組的結果。 例如 **檔** 或 **新聞**。 依預設，Microsoft 搜尋會顯示 **所有** 的縱向、**人員**、**檔案、****網站** 和 **新聞**。  

您可以新增與您組織相關的搜尋行業。 這些會出現在 Microsoft 搜尋結果頁面上 SharePoint、Office 及 Bing。 例如，您可以根據每個部門所需的資訊類型，為行銷相關的內容及另一個銷售人員建立垂直。 您可以新增縱向，只顯示透過連接器編制索引的內容的結果。

您可以在兩個層級建立縱向和結果類型：

- **組織層級**–當使用者從 [SharePoint](https://sharepoint.com/)開始頁面、 [Office](https://office.com)或 [Bing](https://bing.com)進行搜尋時，您在組織層級建立的垂直會出現在搜尋結果頁面上。
...
### <a name="before-you-create-verticals-and-result-types"></a>在您建立縱向和結果類型之前

請考慮下列因素：

- 請確定連接器已編制索引。 這可能需要最多48小時，視檔案大小而定。

- 您無法為位於 SharePoint 中的內容建立垂直。
...
啟動該嚮導之後，您可以逐步指導您定義要搜尋之內容的垂直名稱、內容來源及範圍。

>[!Note]
>會以停用狀態建立縱向。 您可以讓他們查看。

您可以使用一組有限的 [關鍵字查詢語言 (KQL) ](/sharepoint/dev/general-development/keyword-query-language-kql-syntax-reference) 來縮小範圍。  (您可以使用的屬性會在本文稍後說明。 ) 建議您使用具有 boolean 運算子的任意文字關鍵字和屬性限制。 KQL 也支援 [設定檔查詢變數](#profile-query-variables) ，以在垂直方向微調結果。

### <a name="create-a-vertical-at-the-organization-level"></a>在組織層級建立垂直

若要在 SharePoint home、Office 或 Bing 的 Microsoft 搜尋中建立垂直的，請遵循下列步驟：
...
3. 找到 [ **Microsoft 搜尋**] 區段，然後 **為此網站集合選取 [設定 Microsoft 搜尋**]。
4. 在功能窗格中，移至 [ **自訂經驗**]，然後選取 [ **縱向** ] 索引標籤。
5. 若要新增垂直，請選取 [ **新增**]。 或者，若要編輯垂直，請從清單中選取。

## <a name="step-2-create-result-types"></a>步驟2：建立結果類型

您可以使用 *結果類型* 來定義結果在垂直中的顯示方式。 結果配置可讓您直接在搜尋結果中顯示重要資訊，因此使用者不必選取每個結果，即可查看其是否找到所要尋找的專案。

### <a name="default-search-result-layout"></a>預設搜尋結果版面配置

如果在設定連接器時， *標籤* 和 *內容* 屬性已正確對應至來源屬性，則會針對連接器內容顯示預設的搜尋結果版面配置。 *標題* 標籤是最重要的標籤。 *強烈* 建議您將屬性指派給此標籤，以使用預設的搜尋結果版面配置。

### <a name="create-your-own-result-type"></a>建立您自己的結果類型

若要建立您自己的搜尋結果版面配置，並覆寫預設搜尋結果版面配置，您可以建立 *結果類型*。 搜尋結果類型是一種規則，會以不同方式顯示不同類型的搜尋結果。 其包含：

- **一個或多個** 比較每個搜尋結果的條件，例如搜尋結果的內容來源。  
- 用於符合條件之搜尋結果的 **結果版面** 配置。 產生的版面配置會控制符合條件的結果在搜尋結果頁面上顯示和行為的方式。

*如果您未進行適當對應以顯示預設搜尋結果版面配置，您必須至少建立一個結果類型的結果，以顯示在垂直上。* 

您可以為每個垂直方向建立多種結果類型，這可讓您針對不同類型的結果使用不同的版面配置。 例如，您可以自訂 *嚴重性為 1* 的事件，使其具有更醒目的色彩，以及比 *嚴重性 3* 事件更大的字型。
...
2. 若要新增結果類型，請選取 [ **新增**]。 若要編輯結果類型，請選取相關清單中的 [結果類型]。

### <a name="create-a-result-type-at-the-site-level"></a>在網站層級建立結果類型

1. 在您想要結果類型的 [SharePoint](https://sharepoint.com/)網站上，移至 **設定**。
2. 選取 [ **網站資訊**]，然後選取 [ **查看所有網站設定**]。
...
在您啟用垂直時，會有幾個小時的延遲，您才能進行查看。 不過，您可以 `cacheClear=true` 在 SharePoint 和 Office 中附加至 URL，以便立即查看垂直方向。 若為 Bing，請將附加 `&features=uncachedVerticals` 至， `Work vertical URL` 以立即查看垂直。

> [!NOTE]
> 在 https://sharepoint.com/) 從行動網頁瀏覽器中查看時，已新增的縱向 SharePoint] (和[Office](https://office.com)不會顯示。
...
> - 設定檔查詢變數是在 [垂直設定程式](customize-search-page.md#step-1-create-the-search-vertical)的「查詢」區段中定義。
...
|問題  |動作  |
|---------|---------|
| 在垂直上看到「發生錯誤」錯誤訊息。 | 需要有垂直和結果類型，才可完成設定。 請確定您為相同內容來源同時建立這兩者。 |
| 我沒有看到結果版面配置，但我已經建立了它。 | 可能會有幾分鐘的延遲，因為這些設定會進行快取。 請等候幾分鐘後再試一次。        |
